...
/* Define pcap constants if not available in INtime pcap implementation */
#ifndef PCAP_ERROR
#define PCAP_ERROR -1
#endif
...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)
{
   ...
   /* we use pcap socket to send RAW packets */
   *psock = pcap_open_live(ifname, 65536, 1, 0, errbuf);
   if (NULL == *psock)
   {
      ECAT_PRINT_ERROR("interface %s could not open with pcap: %s\n", ifname, errbuf);
      result = 0;
      goto end;
   }

   /* Set packet direction to inbound only */
   if (pcap_setdirection(*psock, PCAP_D_IN) != 0)
   {
      ECAT_PRINT_ERROR("pcap_setdirection failed\n");
      pcap_close(*psock);
      *psock = NULL;
      result = 0;
      goto end;
   ...
   if (pcap_setnonblock(*psock, 1, errbuf) != 0)
   {
      ECAT_PRINT_ERROR("pcap_setnonblock failed: %s\n", errbuf);
      pcap_close(*psock);
      *psock = NULL;
      result = 0;
      goto end;
